...
import { recordSupplierPaymentAction } from "@/actions/payments";
...
export default async function SupplierDetailPage({
  ...
  const [{ data: supplier }, { data: ledger }, { data: branches }] = await Promise.all([
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="mb-1 text-xl font-semibold text-zinc-900 dark:text-zinc-50">{supplier.name}</h1>
      <p className="mb-6 text-sm text-zinc-600 dark:text-zinc-400">
        Balance owed: <span className="font-medium text-zinc-900 dark:text-zinc-50">{balance}</span>
      </p>
